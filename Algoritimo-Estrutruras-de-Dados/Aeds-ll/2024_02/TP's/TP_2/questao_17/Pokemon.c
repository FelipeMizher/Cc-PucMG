...
int comparar(Pokemon *pokemon1, Pokemon *pokemon2){
    int resultado = 0;
    ...
    if(pokemon1->height < pokemon2->height){
        ...
        count++;
        resultado = strcmp(pokemon1->name, pokemon2->name);
    }
    
    return resultado;
}

int getMaiorFilho(Pokemon tmp[], int i, int tamanhoHeap){
    int filho = 0;
    if(2*i == tamanhoHeap || comparar(&tmp[2*i], &tmp[2*i+1]) >= 0){
        filho = 2*i;
    } else{
        filho = 2*i + 1;
    }
...
void reconstruir(Pokemon tmp[], int i, int tamanhoHeap){
...
void construirHeap(Pokemon tmp[], int tamanhoHeap){
    for(int i = tamanhoHeap / 2; i > 0; i--){
        reconstruir(tmp, i, tamanhoHeap);
    }
}
...
void heapsort(){
    ...
    Pokemon tmp[tamSubArray + 1];
    for(int i = 0; i < tamSubArray; i++){
        tmp[i + 1] = sub[i];
    }

    construirHeap(tmp, key);

    for(int i = key + 1; i <= tamSubArray; i++){
        if(comparar(&tmp[i], &tmp[1]) < 0){
            swap(&tmp[i], &tmp[1]);
            reconstruir(tmp, 1, key);
        }
    }
...
void registroLog(){
    ...
    arquivo = fopen("/tmp/821811_heapParcial.txt", "w");
