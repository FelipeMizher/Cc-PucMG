...
int compararData(Pokemon *pokemon1, Pokemon *pokemon2) {
    int resultado = 0;

    if(pokemon1->date.ano < pokemon2->date.ano){
        resultado = -1;
        count++;
    } else if(pokemon1->date.ano > pokemon2->date.ano){
        resultado = 1;
        count++;
    } else if(pokemon1->date.mes < pokemon2->date.mes){
        resultado = -1;
        count++;
    } else if(pokemon1->date.mes > pokemon2->date.mes){
        resultado = 1;
        count++;
    } else if(pokemon1->date.dia < pokemon2->date.dia){
        resultado = -1;
        count++;
    } else if(pokemon1->date.dia > pokemon2->date.dia){
        resultado = 1;
        ...
        if(strcmp(pokemon1->name, pokemon2->name) < 0){
            resultado = -1;
            count++;
        } else if(strcmp(pokemon1->name, pokemon2->name) > 0){
            resultado = 1;
            count++;
        }
    }
...
void insercaoParcial(Pokemon p[]){
    for(int i = 1; i < tamSubArray; i++){ 
        Pokemon tmp = p[i];
        int j = i - 1;
        while((j >= 0) && (compararData(&tmp, &p[j])) == -1){
            p[j + 1] = p[j];
            movimentacoes++;
            j--;
        }
    ...
    for(int i = 0; i < 10 && i < tamSubArray; i++){
        Exibir(&p[i]);
    }
}
...
void registroLog(){
    ...
    arquivo = fopen("/tmp/821811_insecaoParcial.txt", "w");
