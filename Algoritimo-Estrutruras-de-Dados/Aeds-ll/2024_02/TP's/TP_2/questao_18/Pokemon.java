...
class Pokemon{
    ...
    public static void quicksort(Pokemon p[], Log log, int esq, int dir) {
        int i = esq, j = dir;
        Pokemon pivo = p[(dir+esq)/2];
        while (i <= j) {
            while (comparar(p[i], pivo, log)) i++;
            while (comparar(pivo, p[j], log)) j--;
            if (i <= j) {
                swap(p, i, j, log);
                i++;
                j--;
        ...
        if(esq < j){ 
            quicksort(p, log, esq, j);
        }
        if(i < dir){  
            quicksort(p, log, i, dir);
        }               
    }
    ...
    public static void main(String[] args) throws IOException{
        ...
        int tamanho = pokemonArray.length;
        Pokemon.quicksort(pokemonArray, log, 0, tamanho - 1);

        for(int i = 0; i < 10; i++){
            pokemonArray[i].print();
        }
