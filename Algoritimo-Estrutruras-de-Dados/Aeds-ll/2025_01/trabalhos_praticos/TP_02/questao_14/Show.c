...
void radCounting(Show *array, int n, int exp){
    Show output[n];
    int countArray[10] = {0};
    int i;
    ...
    for(i = 0; i < n - 1; i++){
        int j = i + 1;
        while(j < n && array[i].release_year == array[j].release_year){
...
int getMax(Show *array, int n){
    int maior = array[0].release_year;
    int i = 1;

    while(i < n){
        if(array[i].release_year > maior){
            maior = array[i].release_year;
        }
        i++;
    }
