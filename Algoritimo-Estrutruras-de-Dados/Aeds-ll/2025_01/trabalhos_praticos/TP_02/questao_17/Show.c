...
int Comparar(Show *a, Show *b){
    int result = strcmp(a->director, b->director);
    count++;
    if(result == 0){
...
int getMaiorFilho(Show array[], int i, int tamanhoHeap){
    int filho = 0;
    if(2*i == tamanhoHeap || Comparar(&array[2*i], &array[2*i+1]) >= 0){
        filho = 2*i;
    } else {
        filho = 2*i + 1;
    }
...
void reconstruir(Show array[], int i, int tamanhoHeap){
    while(i <= tamanhoHeap / 2){
        int filho = getMaiorFilho(array, i, tamanhoHeap);
...
void construirHeap(Show array[], int tamanhoHeap){
    for(int i = tamanhoHeap / 2; i > 0; i--){
        reconstruir(array, i, tamanhoHeap);
    }
}

void heapSort(Show sub[], int n){
    int key = 10;
    start = clock();
