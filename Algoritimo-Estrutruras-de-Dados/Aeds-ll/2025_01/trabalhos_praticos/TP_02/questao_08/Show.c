...
void Insercao(Show array[], int n, int cor, int h){
    for(int i = h + cor; i < n; i += h){
        ...
        while(j >= 0 && continuar){
            count++;
            int cmpType = compareStrings(array[j].type, tmp.type);
            int cmpTitle = compareStrings(array[j].title, tmp.title);

            if(cmpType > 0 || (cmpType == 0 && cmpTitle > 0)){
                clone(&array[j + h], &array[j]);
                movimentacoes += 3;
                j -= h;
            } else {
                continuar = false;
