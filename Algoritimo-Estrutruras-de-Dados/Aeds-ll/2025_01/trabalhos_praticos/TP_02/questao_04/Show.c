...
void insercao(Show *array, int tam){
    for(int i = 0; i < (tam - 1); i++){
        int menor = i;
        for(int j = i + 1; j < tam; j++){
            if(compareStrings(array[menor].title, array[j].title) > 0){
                menor = j;
            }
        }
        if(menor != i){
...
bool buscarBinaria(Show array[], int n, char *titulo){
    int left = 0, right = n - 1;
    count = 0;

    while(left <= right){
        int mid = (left + right) / 2;
        int cmp = compareStrings(array[mid].title, titulo);
        count++;
...
void Verificar(Show array[], int n){
    char titulo[200];

    start = clock();

    scanf(" %[^\n\r]", titulo);
    while(IsFim(titulo) != 0){
        bool found = buscarBinaria(array, n, titulo);
