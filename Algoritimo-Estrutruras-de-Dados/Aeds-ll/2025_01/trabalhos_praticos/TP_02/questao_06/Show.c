...
void SelecaoRe(Show array[], int index, int tam){
    if(index < tam - 1){
        int menor = index;
        for(int j = index + 1; j < tam; j++){
            if(compareStrings(array[menor].title, array[j].title) > 0){
                menor = j;
            }
            count++;
        }
        Swap(&array[menor], &array[index]);
        movimentacoes += 3;
        SelecaoRe(array, index + 1, tam);
    }
}

void Selecao(Show array[], int n){
    start = clock();
    SelecaoRe(array, 0, n);
    end = clock();
