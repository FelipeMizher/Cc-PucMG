...
void mostrar(Show s[], int n){
    for(int i = 0, pos = n - 1; i < n; i++, pos--){
        Show_Imprimir(&s[i], pos);
    }
}

Show Procurar(Show array[], int tamanho, const char* id) {
    Show copia;
    Show_init(&copia);
...
void metodos(Show array[], int qtd, int N){
    char input[100];
