...
void Show_Imprimir(Show *s, int pos){
    printf("[%d] => %s ## %s ## %s ## %s ## [", pos, s->show_id, s->title, s->type, s->director);
...
int mostrarMedia(){
    ...
    for(int i = primeiro; i != ultimo; i = (i + 1) % 6){
        soma += shows[i].release_year;
        count++;
    }

    if(count != 0){
        resultado = (double)soma / count;
        media = (int)(resultado);
    }
...
void Exibir(){
    ...
    while(i != ultimo){
        Show_Imprimir(&shows[i], j);
        i = (i + 1) % 6;
        j++;
    }
}

Show Procurar(Show array[], const char* id){
    Show copia;
    Show_init(&copia);

    int encontrado = 0;

    for(int i = 0; i < 1368 && !encontrado; i++){
    ...
    if(!encontrado){
        printf("Erro: ID '%s' nao encontrado!\n", id);
    }

    return copia;
}

void metodos(Show array[], int qtd){
    char input[100];
...
int main(){
    ...
    while(fgets(line, sizeof(line), file) && array_count < 1368){
        Show_init(&array[array_count]);
        Show_Ler(&array[array_count], line);
        array_count++;
    }
    fclose(file);
    ...
    scanf("%s", line);
    while(IsFim(line) != 0){
        int index = Pesquisar_ID(array, array_count, line);
        if(index >= 0 && index < array_count){
            Show novo;
            Show_init(&novo);
            clone(&novo, &array[index]);
            inserir(novo);
        }
        scanf(" %s", line); 
    }
