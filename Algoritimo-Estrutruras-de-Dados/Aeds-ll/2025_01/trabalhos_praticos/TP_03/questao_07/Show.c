...
void Show_Imprimir(Show *s, int x){
    printf("[%d] => %s ## %s ## %s ## %s ## [",
           x, s->show_id, s->title, s->type, s->director);
...
void mostrar(){
    int j = 0;
    for(Celula* i = primeiro->prox; i != NULL; i = i->prox){
        Show_Imprimir(&i->elemento, j++);
    }
}

bool pesquisar(Show x){
    bool resp = false;
    Celula* i;

    for(i = primeiro->prox; i != NULL; i = i->prox){
        if(isEqual(i->elemento.show_id, x.show_id) == 1){
            resp = true;
            i = NULL;
        }
    }
...
void metodos(int qtd, Show shows[], int show_count){
    char input[100];
    ...
    for(int i = 0; i < qtd; i++){
        ...
        if(strncmp(input, "I ", 2) == 0){
            sscanf(input + 2, "%s", id);
            int index = Pesquisar_ID(shows, show_count, id);
            if(index >= 0){
                Show_init(&novo);
